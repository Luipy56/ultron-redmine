...
Only Discord user ids stored in `whitelist.json` (under `ULTRON_STATE_DIR`, default `./data`) may use **`/summary`**, **`/ask_issue`**, **`/note`**, **`/ol`**, **`/audit`**, **`/ca`**, **`/list_new_issues`**, **`/issues_by_status`**, **`/list_unassigned_issues`**, **`/find_issue`**, **`/time_summary`**, **`/log_time`**, and **@mention** replies. That directory is listed in `.gitignore`; keep it on the server only.
