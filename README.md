...
- **🧠 Any OpenAI-compatible LLM** — Configure models and endpoints via **`llm_chain`** in **`config.yaml`** (API keys stay in `.env` as the env vars named by each entry’s `api_key_env`). The bot can run **without** an LLM: listing commands and scheduled channel listings still work; `/summary`, `/ask_issue`, and `/note` require a model.
...
| `/find_issue` `text` | Allowlisted — full-text search in the default Redmine project (`redmine.find_issue_project`, default **10_AMVARA**); up to 20 cropped titles + issue links; further matches as issue-number links only |
| `/top_tickets` `project` [`kind_filter`] [`limit`] | Allowlisted — top **open** issues in a project (fuzzy match on identifier/name); `kind_filter` = **priority** (default), **newests**, or **oldests**; `limit` default **10** (max 50) |
| `/new_ticket` `project` `title` `description` | Allowlisted — create a Redmine issue in an **existing** project (fuzzy match); other fields use Redmine defaults; reply includes an issue link |
...
| `/summary`, `/ask_issue`, `/note`, `/ol` | Allowlisted — require a **configured LLM**; if **llm_chain** fails, **cursor-agent** can answer when **`cursor_agent.llm_fallback_enabled`**; **`/ol`** prefers Ollama in **llm_chain** |
| `/audit`, `/ca` | Allowlisted — Amvara server audits (pi + cursor-agent fallback when pi fails or Ollama is busy; **`/ca`** = cursor-agent only) |
...
| `/approve`, `/remove`, `/show_config`, `/pi`, `/upgrade` | Admins (`DISCORD_ADMIN_IDS` or `admins.json`); **`/pi`** runs pi.dev + Ollama on the checkout; **`/upgrade`** queues an **autoagents** FEAT shot (implement → test), dump, Redmine **#7406** note, and verified restart; **`/show_config`** shows non-secret settings (ephemeral) |
...
1. Ensure REST API is enabled and your user can read/update issues in the relevant projects. For **`/log_time`**, the same user must be allowed to **log time** on the target issues (and time tracking must be enabled for the project/activity as usual). For **`/new_ticket`**, the API user must be allowed to **add issues** in the chosen project.
...
- **`/audit host text`** — **Whitelisted.** Runs an **Amvara server audit** on an allowlisted host (pi first; cursor-agent if pi fails or Ollama is busy/unreachable). Agents run on the Ultron host; remote work uses SSH aliases from `~/.ssh/config`. Configure **`amvara.allowed_hosts`** in `config.yaml`.
...
- **`/upgrade text`** — **Admins only.** Creates a **FEAT** under **`autoagents/tasks/`**, runs one autoagents **shot** (implement → test), verifies install, runs **`scripts/ultron-dump.sh`**, posts a journal note to Redmine **#7406**, then **`systemctl restart --no-block`**. Discord reports go to **`reports.channel_id`**. Requires **`cursor_agent.enabled`** (agents still use cursor-agent under the hood). See [docs/agent-loop.md](docs/agent-loop.md).
...
- **`/find_issue` `text`** — Full-text search via Redmine’s project search API in **`redmine.find_issue_project`** (default **`10_AMVARA`**): matches subject, description, notes, and other indexed issue fields. Replies with up to **20** lines of **15-character** subject + issue link; if there are more matches, adds a paragraph of additional issue-number links separated by commas. **Requires a whitelisted user id**; **no LLM** required. Also reachable from **@mention** NL routing.
- **`/top_tickets` `project` [`kind_filter`] [`limit`]** — Lists **open** issues in a Redmine project. **`project`** is an identifier or display name (exact, substring, or fuzzy match — e.g. `93_DIP-RE` resolves to identifier `dip-re`). **`kind_filter`**: **`priority`** (highest first; Redmine priorities Low→Immediate), **`newests`**, or **`oldests`** (default **priority**). **`limit`** default **10**, max **50**. **Requires a whitelisted user id**; **no LLM** required. Also reachable from **@mention** NL routing.
- **`/new_ticket` `project` `title` `description`** — Creates a Redmine **issue** in an **existing** project. **`project`** must resolve to a real project (identifier or name; fuzzy match — the bot will not invent a project). **`title`** is the subject (e.g. `[FOO] Bar`); **`description`** is free text. Other fields use Redmine defaults. The reply includes a markdown link to the new issue. **Requires a whitelisted user id**; **no LLM** required. Also reachable from **@mention** NL routing.
...
Only Discord user ids stored in `whitelist.json` (under `ULTRON_STATE_DIR`, default `./data`) may use **`/summary`**, **`/ask_issue`**, **`/note`**, **`/ol`**, **`/audit`**, **`/ca`**, **`/list_new_issues`**, **`/issues_by_status`**, **`/list_unassigned_issues`**, **`/find_issue`**, **`/top_tickets`**, **`/new_ticket`**, **`/time_summary`**, **`/log_time`**, and **@mention** replies. That directory is listed in `.gitignore`; keep it on the server only.

### Bot admins

Admins may use **`/approve`** to whitelist users, **`/remove`** to drop a user id from the whitelist, **`/show_config`** to inspect safe settings, **`/pi`** to run the pi coding agent on the Ultron checkout, and **`/upgrade`** to run an autoagents FEAT shot with dump, Redmine **#7406** report, and verified restart. An admin is any Discord user id in **`DISCORD_ADMIN_IDS`** and/or **`admins.json`** (same directory as `whitelist.json`, same JSON array-of-integers format as the whitelist). Use the env var for the first admin(s), or create `admins.json` on the server by hand. **Use each user’s numeric Discord id (Developer Mode → copy id);** usernames, display names, or nicknames are not accepted—if `admins.json` contains only invalid entries, no one is an admin except ids in **`DISCORD_ADMIN_IDS`**.
...
If a non-whitelisted user invokes **`/summary`**, **`/ask_issue`**, **`/note`**, **`/ol`**, **`/list_new_issues`**, **`/issues_by_status`**, **`/list_unassigned_issues`**, **`/find_issue`**, **`/top_tickets`**, **`/new_ticket`**, **`/time_summary`**, or **`/log_time`** in a **server channel**, Ultron removes the visible reply so others see nothing. In **DM**, they get a short English message about **`/token`**, asking a bot admin, and contacting the owner. Optional **`BOT_OWNER_CONTACT`** is appended when set.
